...
while true; do
    if test -f ~/$1/$1.mdns.txt; then
        ...
        if [ $skip == 'y' ]; then
            ...
            cat ~/$1/$1.nowildcard.txt | ~/tools/massdns/bin/massdns -t A -r ~/tools/massdns/lists/resolvers.txt -o S -w ~/$1/$1.mdns.txt
...
while read line  ; do out=$(echo $line); gobuster dir -e -u $line -w /usr/share/dirb/wordlists/common.txt -s 200 --wildcard -o ~/$1/gobuster/$out.txt ; done < ~/$1/$1.http.txt
